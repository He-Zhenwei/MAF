...
#ifndef CAFFE_TRIPLET_LAYER_HPP_
#define CAFFE_REIPLET_LAYER_HPP_
...
class TripletMarginLossLayer : public LossLayer<Dtype> {  
 public:  
  explicit TripletMarginLossLayer(const LayerParameter& param)  
 ...
 protected:  
  ...
  Blob<Dtype> diff_samples_; // diff for each sample
  Blob<Dtype> add_diff_vec_; // sum vector for diffs
  Dtype margin;
};
