...
#include "caffe/layers/triplet_loss_margin.hpp"
...
namespace caffe {  
    ...
    void TripletMarginLossLayer<Dtype>::LayerSetUp(  
      ...
      diff_samples_.Reshape(bottom[0]->num(), 1, 1, 1);  
      margin = Dtype(0.1);
    ...
    template <typename Dtype>
    void TripletMarginLossLayer<Dtype>::Reshape(  
      ...
      add_diff_vec_.Reshape(bottom[0]->num(), 1, 1, 1);
      for (int i = 0; i < bottom[0]->num(); ++i)
          add_diff_vec_.mutable_cpu_data()[i] = Dtype(1);
    }
      
    template <typename Dtype>  
    void TripletMarginLossLayer<Dtype>::Forward_cpu(  
        const vector<Blob<Dtype>*>& bottom,  
        const vector<Blob<Dtype>*>& top) {  
...
//      Dtype margin = this->layer_param_.triplet_loss_param().margin();  
      ...
      caffe_sub(dist_sq_ap_.count(), dist_sq_an_.cpu_data(), dist_sq_ap_.cpu_data(), diff_samples_.mutable_cpu_data());
      margin = caffe_cpu_dot(diff_samples_.count(), diff_samples_.cpu_data(), add_diff_vec_.cpu_data())* (-Dtype(1.0) / diff_samples_.count());
      margin = std::max(margin, Dtype(0));
    ...
    template <typename Dtype>  
    void TripletMarginLossLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,  
    ...
    #ifdef CPU_ONLY  
    STUB_GPU(TripletMarginLossLayer);  
    #endif  
      
    INSTANTIATE_CLASS(TripletMarginLossLayer);  
    REGISTER_LAYER_CLASS(TripletMarginLoss);  
