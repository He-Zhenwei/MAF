...
void TripletMarginLossLayer<Dtype>::Forward_gpu(  
  ...
  caffe_gpu_sub(dist_sq_ap_.count(), dist_sq_an_.gpu_data(), dist_sq_ap_.gpu_data(), diff_samples_.mutable_gpu_data());
  margin = caffe_cpu_dot(diff_samples_.count(), diff_samples_.cpu_data(), add_diff_vec_.cpu_data());
  margin = std::max(Dtype(0), -margin* (Dtype(1.0) / diff_samples_.count()));
...
template <typename Dtype>  
void TripletMarginLossLayer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top,  
...
//          static_cast<Dtype>(bottom[0]->num());  
      const Dtype alpha = sign* 1.0 / static_cast<Dtype>(bottom[0]->num());
      if(i==0){
...
INSTANTIATE_LAYER_GPU_FUNCS(TripletMarginLossLayer);
